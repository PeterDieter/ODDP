...
arr_scale=( 1.50 1.75 2.00 ) #1.00 ) #0.90 0.95 1.0 1.05 1.10 1.15 1.20 )
					...
					for bundle in "${arr_bundl[@]}"; do
						...
						for alpha in "${arr_alpha[@]}"; do
							...
							for beta in "${arr_beta[@]}"; do
								...
								stat_file="${grid_inst}_${wait}_${ameth}_${rmeth}_${use_bundling}_${use_alpha}_${use_beta}_${scale}.txt"
								...
								if [[  -z ${use_alpha} && "${alpha}" != "${arr_alpha[0]}" ]]; then
									echo "schedule: ${inst} ${wait} ${ameth} ${rmeth} ${bundl_arg} ${alpha} ${beta} ${scale} --- SKIPPED: Non-w AMeth with an alpha already started"
									continue
								fi
								# skip unnecessary method/beta combinations
								if [[  -z ${use_beta} && "${beta}" != "${arr_beta[0]}" ]]; then
									echo "schedule: ${inst} ${wait} ${ameth} ${rmeth} ${bundl_arg} ${alpha} ${beta} ${scale} --- SKIPPED: Non-l RMeth with an beta already started"
									continue
								fi
								# --------------------------------------------------------
								## specify log files
